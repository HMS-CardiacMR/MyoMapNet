...
def complexBatchStandardize(input, epsilon=1e-4, polar=False):
#     if not polar:
...
#     nn.BatchNorm2d(real_input)
    ...
    if ndims == 5:
        input_axes = [ndims-3,ndims-2] #2,3  ## 2D
        mu = torch.mean(torch.mean(input, 
                                   input_axes[0], True), 
                                   input_axes[1], True)
        input_centered = input - mu
        input_centered_squared = input_centered ** 2
    
        Vrr = torch.mean(torch.mean(input_centered_squared[:,:,:,:,0], 
                                    input_axes[0] ,True),
                                    input_axes[1] ,True) + epsilon
        Vii = torch.mean(torch.mean(input_centered_squared[:,:,:,:,1], 
                                    input_axes[0] ,True),
                                    input_axes[1] ,True) + epsilon
        Vri = torch.mean(torch.mean(input_centered[:,:,:,:,0] * input_centered[:,:,:,:,1],
                                    input_axes[0] ,True), 
                                    input_axes[1] ,True)
        
    elif ndims == 6:
        input_axes = [ndims-4,ndims-3,ndims-2] #2,3,4 ## 3D
        mu = torch.mean(torch.mean(torch.mean(input, 
                                              input_axes[0], True),
                                              input_axes[1], True),
                                              input_axes[2], True)
        ...
        Vrr = torch.mean(torch.mean(torch.mean(input_centered_squared[:,:,:,:,:,0], 
                                               input_axes[0] ,True),
                                               input_axes[1] ,True),
                                               input_axes[2] ,True) + epsilon
        Vii = torch.mean(torch.mean(torch.mean(input_centered_squared[:,:,:,:,:,1], 
                                               input_axes[0] ,True),
                                               input_axes[1] ,True),
                                               input_axes[2] ,True) + epsilon
        Vri = torch.mean(torch.mean(torch.mean(input_centered[:,:,:,:,:,0] * input_centered[:,:,:,:,:,1],
                                               input_axes[0] ,True),
                                               input_axes[1] ,True), 
                                               input_axes[2] ,True)
...
#     print('cmplxBN - stndrzd_output', np.isnan(stndrzd_output.data.cpu()).sum())
...
class ComplexBatchNormalize(nn.Module):
    ...
    def normalizeComplexBatch(self, input):
        ''' normalize each slice alone for 5D input only
            ...
            TODO: handle 3D inputs (input.ndims = 6)
        '''
#         if not self.polar:
#             input = cylindricalToPolarConversion(input)
        ...
        if self.scale:
            stndrzd_output = complexBatchStandardize(input, self.epsilon, not self.polar)
...
#         print('cmplxBN - output', np.isnan(output.data.cpu()).sum())    
        return output 
